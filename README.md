...
Requires the .NET 10 SDK (already installed). The csproj references the game at
`/home/deftones/Apps/vintagestory`.

```bash
./deploy.sh
```

This cleans, builds (`bin/Debug/Mods/FeedOrFallow/FeedOrFallow.dll`), packages
`modinfo.json` + the dll + `assets/` into `FeedOrFallow.zip`, and drops it into
`~/.config/VintagestoryData/Mods/`. Restart the server (or the game's single-player world) to load it.

To just compile without deploying: `dotnet build -c Debug`.
